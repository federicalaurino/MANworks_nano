...
transport3d1d::assembly_mat(void)
{
	...
	*/
	...
	std::cout << "-----------computing peclet tissue.." << std::endl;
	std::cout << "-----------peclet tissue = " << (max_U_t*max_size_t/(2.0*param_transp.At(1))) << std::endl;
...
//estimate_h(meshv, i) 
	...
	std::cout << "-----------computing peclet vessel..." << std::endl;
	std::cout << "-----------peclet vessel = " << (max_U*max_size/(2.0*param_transp.Av(1))) << std::endl;
	...
	for (size_type i=0; i<nb_branches; ++i){
		...
		assem.push_vec(f_Pigreco_scaled);
 		// Project Pi on coefv(PK(1,0)) for export_vtk
		...
		assem.set("m1=data$1(#1);"
			 ...
			 // "m7=data$5(#1);"
			 // "V$7(#3)+=comp(Base(#1).Base(#3))(j,:).m7(j);"
			); 
...
//==================================================================================================
	...
	//The exchange matrices Btt1 and Btv1 should not be scaled by 2*pi*R because we used the same parameter Q used to solve the fluidynamics to build them
	//but Q(fluidynamics)=2*pi*R Lp*P/U, Q(transport)=Lp*P/U in the respective formulations
	//Same thing for Bvt1 and Bvv1 which sould be multiplied by 1/(pi*R*R)
	...
	//gmm::add(gmm::scaled(Bvt1, 2.0/param.R(0)),  	
	  gmm::add(gmm::scaled(Bvt1, 1/(pi*param.R(0)*param.R(0))),							
			  gmm::sub_matrix(AM_transp, 
			  		gmm::sub_interval(dof_transp.Ct(), dof_transp.Cv()),
					gmm::sub_interval(0, dof_transp.Ct())));
	// Copying Bvv
	...
	//gmm::add(gmm::scaled(Bvv1, 2.0/param.R(0)),								
	  gmm::add(gmm::scaled(Bvv1, 1/(pi*param.R(0)*param.R(0))),
			  gmm::sub_matrix(AM_transp, 
					gmm::sub_interval(dof_transp.Ct(), dof_transp.Cv()), 
					gmm::sub_interval(dof_transp.Ct(), dof_transp.Cv()))); 
...
void transport3d1d::update (vector_type Pigreco){
 ...
 bool transport3d1d::solve (void)
 {
	...
	std::cout<<"C_t number of dof.....======="<<dof_transp.Ct()<<std::endl;		
	std::cout<<"mesht number of points.....======="<<mesht.nb_points()<<std::endl;
	std::cout<<"meshv number of points.....======="<<meshv.nb_points()<<std::endl;	
