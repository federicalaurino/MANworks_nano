...
vector_type element_size_t(mesh.dim());
  size_type kk=0;
  for(size_type i=0; i < mesh.dim(); i++){
    element_size_t[kk]=mesh.convex_area_estimate(i,2);
    kk++;
    }
  ...
  max_U_positive_t=*max_element(U.begin(), U.end());
  max_U_negative_t=*min_element(U.begin(), U.end());
  ...
  scalar_type peclet=max_U_t*max_size_t/(2.0*A);
