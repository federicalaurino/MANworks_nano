...
protected:
	...
	//! Monolithic matrix for the network problem
	sparse_matrix_type AM_transp_v;
	//! Array of unknowns for the tissue problem
	vector_type        UM_transp_t;
	//! Array of unknowns for the network problem
	vector_type        UM_transp_v;
	//! Rhs for the tissue problem
	vector_type        FM_transp_t;
	//! Rhs for the network problem
	vector_type        FM_transp_v;
	
	
	//! temporary matrix for update tissue
	sparse_matrix_type AM_temp_t;
	//! temporary right hand side for update tissue
	vector_type        FM_temp_t; 
	//! temporary matrix for update network
	sparse_matrix_type AM_temp_v;
	//! temporary right hand side for update network
	...
	void assembly_mat_tissue_transp(void);
	void assembly_mat_network_transp(void);
	//! Build the monolithic rhs FM_transp by blocks
	void assembly_rhs_tissue_transp(void);
	void assembly_rhs_network_transp(void);
	//! Build the monolithic rhs FM_transp by blocks after Bolus (no injection)
	void assembly_rhs_noinjection(void);
