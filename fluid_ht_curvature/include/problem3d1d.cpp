...
problem3d1d::build_mesh(void)
{
	...
	if(test==0){
		...
		cout << " ---------mesh dim" << mesht.dim() << endl;
		cout << " ---------numeber of convex " << mesht.convex_index().size() << endl;
		cout << " ---------number of points " << mesht.points().size() << endl;
	}
	...
	if(Curve && !Import){
		...
		cout << " ---------numeber of convex " << meshv.convex_index().size() << endl;
		cout << " ---------number of points " << meshv.points().size() << endl;
	}
...
problem3d1d::assembly_rhs(void)
{
	...
	std::cout << "asm_NET_BC DONE" << std::endl;
	gmm::add(Mvv, 
		gmm::sub_matrix(AM,
			gmm::sub_interval(dof.Ut()+dof.Pt(), dof.Uv()),
			gmm::sub_interval(dof.Ut()+dof.Pt(), dof.Uv())));
...
problem3d1d::export_vtk(const string & suff)
{
  if (PARAM.int_value("VTK_EXPORT"))
  {
	...
	if(pf_Ut->is_lagrange()==0){ 
		...
		// Writing on txt file
		std::ofstream Ut_file(descr.OUTPUT+"Ut_P1_c.txt"); 
		Ut_file<<"BEGIN_LIST" << std::endl;
		for(scalar_type i=0;i<mf_P1.nb_dof();i++){
			Ut_file<<Ut_P1[i];
			Ut_file<<std::endl;
		}
		Ut_file<< "END_LIST" << std::endl;
		...
		// Writing on txt file
		std::ofstream Ut_file(descr.OUTPUT+"Ut_c.txt"); 
		Ut_file<<"BEGIN_LIST" << std::endl;
		for(scalar_type i=0;i<mf_Ut.nb_dof();i++){
			Ut_file<<Ut[i];
			Ut_file<<std::endl;
		}
		Ut_file<< "END_LIST" << std::endl; 
	...
	// Writing on txt file
	std::ofstream Pt_file(descr.OUTPUT+"Pt_c.txt"); 
	Pt_file<<"BEGIN_LIST" << std::endl;
	for(scalar_type i=0;i<mf_Pt.nb_dof();i++){
		Pt_file<<Pt[i];
		Pt_file<<std::endl;
	}
	Pt_file<< "END_LIST" << std::endl;
...
// ----------------------
	...
	last_dof=nb_branches*mf_Uvi[nb_branches-1].nb_dof();
		...
		for(size_type i=0; i<nb_branches; ++i){
			...
			std::ofstream Uvc_file(descr.OUTPUT+"Uv_c"+suff+std::to_string(i)+".txt"); 
			Uvc_file<<"BEGIN_LIST" << std::endl;
			for(scalar_type k=0; k < mf_Uvi[i].nb_dof(); k++){
				Uvc_file<<Uv[start + k];
				Uvc_file<<std::endl;
			}
			Uvc_file<< "END_LIST" << std::endl;
		}
	}
	...
	// Writing on txt file
	std::ofstream Pv_file(descr.OUTPUT+"Pv_c.txt"); 
	Pv_file<<"BEGIN_LIST" << std::endl;
	for(scalar_type i=0;i<mf_Pv.nb_dof();i++){
		Pv_file<<Pv[i];
		Pv_file<<std::endl;
	}
	Pv_file<< "END_LIST" << std::endl;
