...
Num_el_yz= {8.0, Name "Number of elements for unit size"}
Num_el_x= {8.0, Name "Number of elements for unit size"}
];

// mesh element size at each point
cellsize=1./Num_el_yz;
...
p=newp;
Point(p)  ={x0,yc,zc, cellsize};
Point(p+1)={x0,yc,zc+R, cellsize};
Point(p+2)={x0,yc+R,zc, cellsize};
Point(p+3)={x0,yc,zc-R, cellsize};
Point(p+4)={x0,yc-R,zc, cellsize};
...
// Extrude
cellsize_x=1./Num_el_x;
tmp1[]=Extrude {cellsize_x, 0, 0} { Surface{s+0};};
ss1=tmp1[1];
tmp2[]=Extrude {cellsize_x, 0, 0} { Surface{s+1};};
ss2=tmp2[1];
 
Characteristic Length(10)=cellsize_x;
Characteristic Length(11)=cellsize_x;
Characteristic Length(15)=cellsize_x;
Characteristic Length(19)=cellsize_x;

tmp3[]=Extrude {x1-x0-cellsize_x, 0, 0} { Surface{54};};
tmp4[]=Extrude {x1-x0-cellsize_x, 0, 0} { Surface{76};};


//Define physical volume
v=newv;
Physical Volume(v) = {1,2,3,4}; 
